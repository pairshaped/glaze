...
-spec method_override(gleam@http@request:request(PXQ)) -> gleam@http@request:request(PXQ).
...
-spec sort_keys(list({binary(), PZH})) -> list({binary(), PZH}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, PYB} | {error, any()}) -> {ok, PYB} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((PYP) -> {ok, PYQ} | {error, any()}),
    binary()
) -> fun((PYP) -> {ok, PYQ} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((PYJ, bitstring()) -> {ok, PYJ} |
        {error, gleam@http@response:response(body())}),
    PYJ
) -> {ok, {gleam@option:option(buffered_reader()), integer(), PYJ}} |
    {error, gleam@http@response:response(body())}.
