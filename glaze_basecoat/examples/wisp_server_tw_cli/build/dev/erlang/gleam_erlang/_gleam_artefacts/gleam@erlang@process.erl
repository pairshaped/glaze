...
-opaque subject(EEK) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EEK)}.

-type name(EEL) :: any() | {gleam_phantom, EEL}.

-type do_not_leak() :: any().

-type selector(EEM) :: any() | {gleam_phantom, EEM}.
...
-spec 'receive'(subject(EFK), integer()) -> {ok, EFK} | {error, nil}.
...
-spec receive_forever(subject(EFS)) -> EFS.
...
-spec selector_receive(selector(EFW), integer()) -> {ok, EFW} | {error, nil}.
...
-spec selector_receive_forever(selector(EGA)) -> EGA.
...
-spec map_selector(selector(EGC), fun((EGC) -> EGE)) -> selector(EGE).
map_selector(A, B) ->
...
-spec merge_selector(selector(EGG), selector(EGG)) -> selector(EGG).
merge_selector(A, B) ->
...
-spec select_map(selector(EGR), subject(EGT), fun((EGT) -> EGR)) -> selector(EGR).
...
-spec select(selector(EGN), subject(EGN)) -> selector(EGN).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EHB)
) -> selector(EHB).
...
-spec select_other(selector(EHF), fun((gleam@dynamic:dynamic_()) -> EHF)) -> selector(EHF).
...
-spec deselect(selector(EGW), subject(any())) -> selector(EGW).
...
-spec select_specific_monitor(selector(EHR), monitor(), fun((down()) -> EHR)) -> selector(EHR).
...
-spec select_monitors(selector(EHU), fun((down()) -> EHU)) -> selector(EHU).
...
-spec select_trapped_exits(selector(EGK), fun((exit_message()) -> EGK)) -> selector(EGK).
...
-spec deselect_specific_monitor(selector(EHX), monitor()) -> selector(EHX).
...
-spec send_after(subject(EIT), integer(), EIT) -> timer().
...
-spec perform_call(
    subject(EIA),
    fun((subject(EIC)) -> EIA),
    fun((selector(EIC)) -> {ok, EIC} | {error, nil})
) -> EIC.
...
-spec call(subject(EIH), integer(), fun((subject(EIJ)) -> EIH)) -> EIJ.
...
-spec call_forever(subject(EIL), fun((subject(EIN)) -> EIL)) -> EIN.
