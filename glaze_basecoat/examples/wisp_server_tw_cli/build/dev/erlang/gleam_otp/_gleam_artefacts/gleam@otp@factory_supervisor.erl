...
-opaque supervisor(FME, FMF) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(FME, FMF))}.

-type message(FMG, FMH) :: any() | {gleam_phantom, FMG, FMH}.

-opaque builder(FMI, FMJ) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((FMI) -> {ok, gleam@otp@actor:started(FMJ)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(FMI, FMJ)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(FMK, FML) :: {local,
        gleam@erlang@process:name(message(FMK, FML))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(FMM) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, FMM}.
...
-type erlang_child_spec_property(FMN, FMO) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((FMN) -> {ok, gleam@otp@actor:started(FMO)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(FMP, FMQ))) -> supervisor(FMP, FMQ).
get_by_name(Name) ->
...
-spec named(builder(FNG, FNH), gleam@erlang@process:name(message(FNG, FNH))) -> builder(FNG, FNH).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(FNP, FNQ), integer(), integer()) -> builder(FNP, FNQ).
...
-spec timeout(builder(FNV, FNW), integer()) -> builder(FNV, FNW).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(FOB, FOC), gleam@otp@supervision:restart()) -> builder(FOB, FOC).
...
-spec start(builder(FOH, FOI)) -> {ok,
        gleam@otp@actor:started(supervisor(FOH, FOI))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(FPG, FPH)) -> gleam@otp@supervision:child_specification(supervisor(FPG, FPH)).
supervised(Builder) ->
...
-spec start_child(supervisor(FPN, FPO), FPN) -> {ok,
        gleam@otp@actor:started(FPO)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((FQL) -> {ok, gleam@otp@actor:started(FQM)} |
        {error, gleam@otp@actor:start_error()}),
    FQL
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), FQM, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((FMW) -> {ok, gleam@otp@actor:started(FMX)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FMW, FMX).
...
-spec supervisor_child(
    fun((FNB) -> {ok, gleam@otp@actor:started(FNC)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FNB, FNC).
