...
-type patch(JFW) :: {patch,
        integer(),
        integer(),
        list(change(JFW)),
        list(patch(JFW))}.

-type change(JFX) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(JFX)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(JFX)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(JFY)), list(patch(JFY))) -> patch(JFY).
...
?DOC(false).
-spec add_child(patch(JHB), patch(JHB)) -> patch(JHB).
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(JJS),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JJS)), lustre@vdom@vnode:element(JJS))
) -> gleam@json:json().
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(JJW)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JJW)), lustre@vdom@vnode:element(JJW))
) -> gleam@json:json().
...
-spec change_to_json(
    change(JHI),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JHI)), lustre@vdom@vnode:element(JHI))
) -> gleam@json:json().
...
-spec to_json(
    patch(JHF),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JHF)), lustre@vdom@vnode:element(JHF))
) -> gleam@json:json().
to_json(Patch, Memos) ->
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(JGI)),
    list(lustre@vdom@vattr:attribute(JGI))
) -> change(JGI).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(JGS)) -> change(JGS).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(JGV)), integer()) -> change(JGV).
