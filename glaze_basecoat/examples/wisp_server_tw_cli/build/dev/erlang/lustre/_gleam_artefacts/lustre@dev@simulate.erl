...
-opaque app(NQU, NQV, NQW) :: {app,
        fun((NQU) -> {NQV, lustre@effect:effect(NQW)}),
        fun((NQV, NQW) -> {NQV, lustre@effect:effect(NQW)}),
        fun((NQV) -> lustre@vdom@vnode:element(NQW))}.

-opaque simulation(NQX, NQY) :: {simulation,
        fun((NQX, NQY) -> {NQX, lustre@effect:effect(NQY)}),
        fun((NQX) -> lustre@vdom@vnode:element(NQY)),
        list(event(NQY)),
        NQX,
        lustre@vdom@vnode:element(NQY)}.

-type event(NQZ) :: {dispatch, NQZ} |
...
-spec simple(
    fun((NRA) -> NRB),
    fun((NRB, NRC) -> NRB),
    fun((NRB) -> lustre@vdom@vnode:element(NRC))
) -> app(NRA, NRB, NRC).
...
-spec application(
    fun((NRH) -> {NRI, lustre@effect:effect(NRJ)}),
    fun((NRI, NRJ) -> {NRI, lustre@effect:effect(NRJ)}),
    fun((NRI) -> lustre@vdom@vnode:element(NRJ))
) -> app(NRH, NRI, NRJ).
...
-spec start(app(NRQ, NRR, NRS), NRQ) -> simulation(NRR, NRS).
start(App, Args) ->
...
-spec message(simulation(NRY, NRZ), NRZ) -> simulation(NRY, NRZ).
...
-spec problem(simulation(NTE, NTF), binary(), binary()) -> simulation(NTE, NTF).
...
-spec model(simulation(NTK, any())) -> NTK.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), NTP)) -> lustre@vdom@vnode:element(NTP).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), NTU)) -> list(event(NTU)).
history(Simulation) ->
...
-spec click(simulation(NSL, NSM), lustre@dev@query:'query'()) -> simulation(NSL, NSM).
...
-spec input(simulation(NSR, NSS), lustre@dev@query:'query'(), binary()) -> simulation(NSR, NSS).
