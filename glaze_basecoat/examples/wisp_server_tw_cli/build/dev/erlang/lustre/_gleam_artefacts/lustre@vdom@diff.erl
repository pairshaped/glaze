...
-type diff(KHR) :: {diff,
        lustre@vdom@patch:patch(KHR),
        lustre@vdom@cache:cache(KHR)}.

-type partial_diff(KHS) :: {partial_diff,
        lustre@vdom@patch:patch(KHS),
        lustre@vdom@cache:cache(KHS),
        lustre@vdom@cache:events(KHS)}.

-type attribute_change(KHT) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(KHT)),
        list(lustre@vdom@vattr:attribute(KHT)),
        lustre@vdom@cache:events(KHT)}.
...
-spec diff_attributes(
    boolean(),
    lustre@vdom@path:path(),
    lustre@vdom@cache:events(KIW),
    list(lustre@vdom@vattr:attribute(KIW)),
    list(lustre@vdom@vattr:attribute(KIW)),
    list(lustre@vdom@vattr:attribute(KIW)),
    list(lustre@vdom@vattr:attribute(KIW))
) -> attribute_change(KIW).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(KHZ)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(KHZ)),
    list(lustre@vdom@vnode:element(KHZ)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(KHZ)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(KHZ)),
    list(lustre@vdom@patch:patch(KHZ)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(KHZ),
    lustre@vdom@cache:events(KHZ)
) -> partial_diff(KHZ).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(KHU),
    lustre@vdom@vnode:element(KHU),
    lustre@vdom@vnode:element(KHU)
) -> diff(KHU).
diff(Cache, Old, New) ->
