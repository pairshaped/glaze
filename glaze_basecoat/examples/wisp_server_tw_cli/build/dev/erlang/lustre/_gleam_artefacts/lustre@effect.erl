...
-opaque effect(HCA) :: {effect,
        list(fun((actions(HCA)) -> nil)),
        list(fun((actions(HCA)) -> nil)),
        list(fun((actions(HCA)) -> nil))}.

-type actions(HCB) :: {actions,
        fun((HCB) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(HCB)) -> nil),
...
-spec do_comap_select(
    actions(HDL),
    gleam@erlang@process:selector(HDN),
    fun((HDN) -> HDL)
) -> nil.
...
-spec do_comap_actions(actions(HDH), fun((HDJ) -> HDH)) -> actions(HDJ).
...
-spec do_map(list(fun((actions(HDB)) -> nil)), fun((HDB) -> HDE)) -> list(fun((actions(HDE)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(HCX), fun((HCX) -> HCZ)) -> effect(HCZ).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((HDP) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(HDP)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((HCE) -> nil)) -> nil)) -> effect(HCE).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((HCG) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(HCG).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((HCI) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(HCI).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((HCM) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(HCM))
) -> effect(HCM).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(HCT))) -> effect(HCT).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
