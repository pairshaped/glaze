...
-type runtime(MSS) :: any() | {gleam_phantom, MSS}.
...
-spec application(
    fun((MTO) -> {MTP, lustre@effect:effect(MTQ)}),
    fun((MTP, MTQ) -> {MTP, lustre@effect:effect(MTQ)}),
    fun((MTP) -> lustre@vdom@vnode:element(MTQ))
) -> lustre@runtime@app:app(MTO, MTP, MTQ).
...
-spec element(lustre@vdom@vnode:element(MTB)) -> lustre@runtime@app:app(any(), nil, MTB).
element(View) ->
    application(
...
-spec simple(
    fun((MTH) -> MTI),
    fun((MTI, MTJ) -> MTI),
    fun((MTI) -> lustre@vdom@vnode:element(MTJ))
) -> lustre@runtime@app:app(MTH, MTI, MTJ).
...
-spec component(
    fun((MTX) -> {MTY, lustre@effect:effect(MTZ)}),
    fun((MTY, MTZ) -> {MTY, lustre@effect:effect(MTZ)}),
    fun((MTY) -> lustre@vdom@vnode:element(MTZ)),
    list(lustre@runtime@app:option(MTZ))
) -> lustre@runtime@app:app(MTX, MTY, MTZ).
...
-spec named(
    lustre@runtime@app:app(MUI, MUJ, MUK),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(MUK))
) -> lustre@runtime@app:app(MUI, MUJ, MUK).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(MVC, any(), MVE), binary(), MVC) -> {ok,
        runtime(MVE)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec supervised(lustre@runtime@app:app(MVU, any(), MVW), MVU) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MVW))).
...
-spec factory(lustre@runtime@app:app(MWD, any(), MWF)) -> gleam@otp@factory_supervisor:builder(MWD, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MWF))).
factory(App) ->
...
-spec send(runtime(MWU), lustre@runtime@server@runtime:message(MWU)) -> nil.
...
-spec dispatch(MWX) -> lustre@runtime@server@runtime:message(MWX).
dispatch(Message) ->
...
-spec start(lustre@runtime@app:app(MUT, any(), MUV), binary(), MUT) -> {ok,
        runtime(MUV)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(MVL, any(), MVN), MVL) -> {ok,
        runtime(MVN)} |
    {error, error()}.
