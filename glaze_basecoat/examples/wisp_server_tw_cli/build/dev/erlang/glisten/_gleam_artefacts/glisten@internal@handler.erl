...
-type message(AAVA) :: {internal, internal_message()} | {user, AAVA}.

-type loop_message(AAVB) :: {packet, bitstring()} | {custom, AAVB}.

-type loop_state(AAVC, AAVD) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(AAVD)),
        glisten@transport:transport(),
        AAVC}.

-type connection(AAVE) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(AAVE))}.

-type next(AAVF, AAVG) :: {continue,
        AAVF,
        gleam@option:option(gleam@erlang@process:selector(AAVG))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(AAVH, AAVI) :: {handler,
        glisten@socket:socket(),
        fun((AAVH, loop_message(AAVI), connection(AAVI)) -> next(AAVH, loop_message(AAVI))),
        fun((connection(AAVI)) -> {AAVH,
            gleam@option:option(gleam@erlang@process:selector(AAVI))}),
        gleam@option:option(fun((AAVH) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(AAVV) -> next(AAVV, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(AAVZ, AAWA), gleam@erlang@process:selector(AAWA)) -> next(AAVZ, AAWA).
...
?DOC(false).
-spec start(handler(any(), AAWP)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(AAWP)))} |
