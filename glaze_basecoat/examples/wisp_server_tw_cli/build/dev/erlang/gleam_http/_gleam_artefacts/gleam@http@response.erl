...
-type response(VKN) :: {response, integer(), list({binary(), binary()}), VKN}.
...
-spec set_header(response(VLC), binary(), binary()) -> response(VLC).
...
-spec prepend_header(response(VLF), binary(), binary()) -> response(VLF).
...
-spec set_body(response(any()), VLK) -> response(VLK).
...
-spec try_map(response(VKO), fun((VKO) -> {ok, VKQ} | {error, VKR})) -> {ok,
        response(VKQ)} |
    {error, VKR}.
...
-spec map(response(VLM), fun((VLM) -> VLO)) -> response(VLO).
...
-spec expire_cookie(response(VLX), binary(), gleam@http@cookie:attributes()) -> response(VLX).
