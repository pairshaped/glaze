...
-spec set_header(request(VAT), binary(), binary()) -> request(VAT).
...
-spec prepend_header(request(VAW), binary(), binary()) -> request(VAW).
...
-spec set_body(request(any()), VBB) -> request(VBB).
set_body(Req, Body) ->
    {request,
...
-spec map(request(VBD), fun((VBD) -> VBF)) -> request(VBF).
...
-spec set_query(request(VBP), list({binary(), binary()})) -> request(VBP).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(VBT), gleam@http:method()) -> request(VBT).
...
-spec set_scheme(request(VCA), gleam@http:scheme()) -> request(VCA).
...
-spec set_host(request(VCD), binary()) -> request(VCD).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(VCG), integer()) -> request(VCG).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(VCJ), binary()) -> request(VCJ).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(VCM), binary(), binary()) -> request(VCM).
...
-spec remove_cookie(request(VCS), binary()) -> request(VCS).
