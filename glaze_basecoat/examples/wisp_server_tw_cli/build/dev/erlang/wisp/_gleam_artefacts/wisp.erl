...
-spec method_override(gleam@http@request:request(AKEL)) -> gleam@http@request:request(AKEL).
...
-spec sort_keys(list({binary(), AKGC})) -> list({binary(), AKGC}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, AKEW} | {error, any()}) -> {ok, AKEW} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((AKFK) -> {ok, AKFL} | {error, any()}),
    binary()
) -> fun((AKFK) -> {ok, AKFL} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AKFE, bitstring()) -> {ok, AKFE} |
        {error, gleam@http@response:response(body())}),
    AKFE
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AKFE}} |
    {error, gleam@http@response:response(body())}.
