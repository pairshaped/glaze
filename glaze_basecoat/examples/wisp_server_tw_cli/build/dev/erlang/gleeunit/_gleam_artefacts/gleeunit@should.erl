...
-spec be_ok({ok, XPX} | {error, any()}) -> XPX.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, XQC}) -> XQC.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(XQF)) -> XQF.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
