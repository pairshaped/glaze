...
-opaque decoder(BWY) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BWY, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BXG)) -> {ok, BXG} |
...
-spec map(decoder(CBE), fun((CBE) -> CBG)) -> decoder(CBG).
...
-spec then(decoder(CBQ), fun((CBQ) -> decoder(CBS))) -> decoder(CBS).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CBV), list(decoder(CBV))) -> decoder(CBV).
...
-spec recursive(fun(() -> decoder(CCL))) -> decoder(CCL).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CBA)) -> decoder(gleam@option:option(CBA)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BZB} | {error, BZB})
) -> {BZB, list(decode_error())}.
...
-spec collapse_errors(decoder(CBN), binary()) -> decoder(CBN).
...
-spec failure(CCF, binary()) -> decoder(CCF).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CCH} | {error, CCH})
) -> decoder(CCH).
...
-spec fold_dict(
    {gleam@dict:dict(CAM, CAN), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CAM, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CAN, list(decode_error())})
) -> {gleam@dict:dict(CAM, CAN), list(decode_error())}.
...
-spec dict(decoder(CAF), decoder(CAH)) -> decoder(gleam@dict:dict(CAF, CAH)).
dict(Key, Value) ->
...
-spec list(decoder(BZT)) -> decoder(list(BZT)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BYC, list(decode_error())}, list(any())) -> {BYC,
    list(decode_error())}.
...
-spec index(
    list(BXQ),
    list(BXQ),
    fun((gleam@dynamic:dynamic_()) -> {BXT, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BXQ)) -> {BXT, list(decode_error())})
...
-spec subfield(list(any()), decoder(BXB), fun((BXB) -> decoder(BXD))) -> decoder(BXD).
...
-spec at(list(any()), decoder(BXN)) -> decoder(BXN).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BYL), fun((BYL) -> decoder(BYN))) -> decoder(BYN).
...
-spec optional_field(any(), BYR, decoder(BYR), fun((BYR) -> decoder(BYT))) -> decoder(BYT).
...
-spec optionally_at(list(any()), BYY, decoder(BYY)) -> decoder(BYY).
