...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EOD)) -> {ok, EOD} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(ENX)) -> {ok, ENX} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(ENT)) -> {ok, ENT} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EOJ), fun((EOJ) -> json())) -> json().
...
-spec array(list(EON), fun((EON) -> json())) -> json().
