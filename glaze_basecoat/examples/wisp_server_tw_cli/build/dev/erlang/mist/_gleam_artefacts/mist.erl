...
-opaque next(AFSI, AFSJ) :: {continue,
        AFSI,
        gleam@option:option(gleam@erlang@process:selector(AFSJ))} |
    normal_stop |
...
-opaque builder(AFSK, AFSL) :: {builder,
        integer(),
        fun((gleam@http@request:request(AFSK)) -> gleam@http@response:response(AFSL)),
...
-type websocket_message(AFSM) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, AFSM}.
...
-spec continue(AFSN) -> next(AFSN, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(AFSR, AFSS), gleam@erlang@process:selector(AFSS)) -> next(AFSR, AFSS).
...
-spec convert_next(next(AFTG, AFTH)) -> mist@internal@next:next(AFTG, AFTH).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(AFUI)) -> gleam@http@response:response(AFUK))
) -> builder(AFUI, AFUK).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(AFUO, AFUP), integer()) -> builder(AFUO, AFUP).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), AFUU),
    integer(),
    gleam@http@response:response(AFUU)
) -> builder(mist@internal@http:connection(), AFUU).
...
-spec bind(builder(AFVG, AFVH), binary()) -> builder(AFVG, AFVH).
...
-spec with_ipv6(builder(AFVM, AFVN)) -> builder(AFVM, AFVN).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(AFVS, AFVT), binary(), binary()) -> builder(AFVS, AFVT).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AFWI)
) -> {ok, websocket_message(AFWI)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AFWO, websocket_message(AFWP), mist@internal@websocket:websocket_connection()) -> next(AFWO, AFWP)),
    fun((mist@internal@websocket:websocket_connection()) -> {AFWO,
        gleam@option:option(gleam@erlang@process:selector(AFWP))}),
    fun((AFWO) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(AFXD)) -> {ok,
            gleam@otp@actor:initialised(AFXF, AFXD, any())} |
        {error, binary()}),
    fun((AFXF, AFXD, s_s_e_connection()) -> gleam@otp@actor:next(AFXF, AFXD))
) -> gleam@http@response:response(response_data()).
