...
-type valid_message(ADJW) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, ADJW}.

-type websocket_message(ADJX) :: {valid, valid_message(ADJX)} | invalid.
...
-type handler_message(ADJY) :: {internal, gramps@websocket:frame()} |
    {user, ADJY}.

-type websocket_state(ADJZ) :: {websocket_state,
        bitstring(),
        ADJZ,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(ADLF))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(ADLF))).
...
-spec apply_frames(
    ...
    fun((ADKV, handler_message(ADKW), websocket_connection()) -> mist@internal@next:next(ADKV, ADKW)),
    websocket_connection(),
    mist@internal@next:next(ADKV, websocket_message(ADKW)),
    fun((ADKV) -> nil)
) -> mist@internal@next:next(ADKV, websocket_message(ADKW)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {ADKI,
        gleam@option:option(gleam@erlang@process:selector(ADKJ))}),
    fun((ADKI) -> nil),
    fun((ADKI, handler_message(ADKJ), websocket_connection()) -> mist@internal@next:next(ADKI, ADKJ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(ADKJ)))} |
    {error, nil}.
