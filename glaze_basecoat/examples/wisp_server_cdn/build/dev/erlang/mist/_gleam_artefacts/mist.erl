...
-opaque next(ADYG, ADYH) :: {continue,
        ADYG,
        gleam@option:option(gleam@erlang@process:selector(ADYH))} |
    normal_stop |
...
-opaque builder(ADYI, ADYJ) :: {builder,
        integer(),
        fun((gleam@http@request:request(ADYI)) -> gleam@http@response:response(ADYJ)),
...
-type websocket_message(ADYK) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, ADYK}.
...
-spec continue(ADYL) -> next(ADYL, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(ADYP, ADYQ), gleam@erlang@process:selector(ADYQ)) -> next(ADYP, ADYQ).
...
-spec convert_next(next(ADZE, ADZF)) -> mist@internal@next:next(ADZE, ADZF).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(AEAG)) -> gleam@http@response:response(AEAI))
) -> builder(AEAG, AEAI).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(AEAM, AEAN), integer()) -> builder(AEAM, AEAN).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), AEAS),
    integer(),
    gleam@http@response:response(AEAS)
) -> builder(mist@internal@http:connection(), AEAS).
...
-spec bind(builder(AEBE, AEBF), binary()) -> builder(AEBE, AEBF).
...
-spec with_ipv6(builder(AEBK, AEBL)) -> builder(AEBK, AEBL).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(AEBQ, AEBR), binary(), binary()) -> builder(AEBQ, AEBR).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AECG)
) -> {ok, websocket_message(AECG)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AECM, websocket_message(AECN), mist@internal@websocket:websocket_connection()) -> next(AECM, AECN)),
    fun((mist@internal@websocket:websocket_connection()) -> {AECM,
        gleam@option:option(gleam@erlang@process:selector(AECN))}),
    fun((AECM) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(AEDB)) -> {ok,
            gleam@otp@actor:initialised(AEDD, AEDB, any())} |
        {error, binary()}),
    fun((AEDD, AEDB, s_s_e_connection()) -> gleam@otp@actor:next(AEDD, AEDB))
) -> gleam@http@response:response(response_data()).
