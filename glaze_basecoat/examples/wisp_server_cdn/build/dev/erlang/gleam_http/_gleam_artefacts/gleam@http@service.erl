...
-spec map_response_body(
    fun((VAI) -> gleam@http@response:response(VAJ)),
    fun((VAJ) -> VAL)
) -> fun((VAI) -> gleam@http@response:response(VAL)).
...
-spec prepend_response_header(
    fun((VAN) -> gleam@http@response:response(VAO)),
    binary(),
    binary()
) -> fun((VAN) -> gleam@http@response:response(VAO)).
...
-spec ensure_post(gleam@http@request:request(VAR)) -> {ok,
        gleam@http@request:request(VAR)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(VBA)) -> VBC)) -> fun((gleam@http@request:request(VBA)) -> VBC).
