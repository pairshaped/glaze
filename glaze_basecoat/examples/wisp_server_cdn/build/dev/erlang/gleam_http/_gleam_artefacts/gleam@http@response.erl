...
-type response(UWM) :: {response, integer(), list({binary(), binary()}), UWM}.
...
-spec set_header(response(UXB), binary(), binary()) -> response(UXB).
...
-spec prepend_header(response(UXE), binary(), binary()) -> response(UXE).
...
-spec set_body(response(any()), UXJ) -> response(UXJ).
...
-spec try_map(response(UWN), fun((UWN) -> {ok, UWP} | {error, UWQ})) -> {ok,
        response(UWP)} |
    {error, UWQ}.
...
-spec map(response(UXL), fun((UXL) -> UXN)) -> response(UXN).
...
-spec expire_cookie(response(UXW), binary(), gleam@http@cookie:attributes()) -> response(UXW).
