...
-spec set_header(request(UMS), binary(), binary()) -> request(UMS).
...
-spec prepend_header(request(UMV), binary(), binary()) -> request(UMV).
...
-spec set_body(request(any()), UNA) -> request(UNA).
set_body(Req, Body) ->
    {request,
...
-spec map(request(UNC), fun((UNC) -> UNE)) -> request(UNE).
...
-spec set_query(request(UNO), list({binary(), binary()})) -> request(UNO).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(UNS), gleam@http:method()) -> request(UNS).
...
-spec set_scheme(request(UNZ), gleam@http:scheme()) -> request(UNZ).
...
-spec set_host(request(UOC), binary()) -> request(UOC).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(UOF), integer()) -> request(UOF).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(UOI), binary()) -> request(UOI).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(UOL), binary(), binary()) -> request(UOL).
...
-spec remove_cookie(request(UOR), binary()) -> request(UOR).
