...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(ENA)) -> {ok, ENA} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EMU)) -> {ok, EMU} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EMQ)) -> {ok, EMQ} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(ENG), fun((ENG) -> json())) -> json().
...
-spec array(list(ENK), fun((ENK) -> json())) -> json().
