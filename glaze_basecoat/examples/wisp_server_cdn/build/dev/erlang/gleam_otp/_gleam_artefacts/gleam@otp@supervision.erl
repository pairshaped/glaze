...
-type child_specification(FIN) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FIN)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FIO)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FIO).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FIT)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FIT).
supervisor(Start) ->
...
-spec significant(child_specification(FIY), boolean()) -> child_specification(FIY).
...
-spec timeout(child_specification(FJB), integer()) -> child_specification(FJB).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FJE), restart()) -> child_specification(FJE).
...
-spec map_data(child_specification(FJH), fun((FJH) -> FJJ)) -> child_specification(FJJ).
