...
-type message(EPI) :: {message, EPI} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EPJ, EPK) :: {continue,
        EPJ,
        gleam@option:option(gleam@erlang@process:selector(EPK))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EPL, EPM) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EPL,
        gleam@erlang@process:selector(message(EPM)),
        gleam@otp@system:debug_state(),
        fun((EPL, EPM) -> next(EPL, EPM))}.

-type started(EPN) :: {started, gleam@erlang@process:pid_(), EPN}.

-opaque initialised(EPO, EPP, EPQ) :: {initialised,
        EPO,
        gleam@option:option(gleam@erlang@process:selector(EPP)),
        EPQ}.

-opaque builder(EPR, EPS, EPT) :: {builder,
        fun((gleam@erlang@process:subject(EPS)) -> {ok,
                initialised(EPR, EPS, EPT)} |
            {error, binary()}),
        integer(),
        fun((EPR, EPS) -> next(EPR, EPS)),
        gleam@option:option(gleam@erlang@process:name(EPS))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EPU) :: {ack, {ok, EPU} | {error, binary()}} |
...
-spec continue(EPZ) -> next(EPZ, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(EQL, EQM), gleam@erlang@process:selector(EQM)) -> next(EQL, EQM).
...
-spec initialised(EQS) -> initialised(EQS, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(EQX, any(), EQZ),
    gleam@erlang@process:selector(ERD)
) -> initialised(EQX, ERD, EQZ).
...
-spec returning(initialised(ERI, ERJ, any()), ERO) -> initialised(ERI, ERJ, ERO).
...
-spec new(ERS) -> builder(ERS, ERT, gleam@erlang@process:subject(ERT)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(ERY)) -> {ok, initialised(ESA, ERY, ESB)} |
        {error, binary()})
) -> builder(ESA, ERY, ESB).
...
-spec on_message(builder(ESK, ESL, ESM), fun((ESK, ESL) -> next(ESK, ESL))) -> builder(ESK, ESL, ESM).
...
-spec named(builder(ESV, ESW, ESX), gleam@erlang@process:name(ESW)) -> builder(ESV, ESW, ESX).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(ETK))) -> gleam@erlang@process:selector(message(ETK)).
...
-spec receive_message(self(any(), ETG)) -> message(ETG).
...
-spec initialise_actor(
    builder(any(), any(), EUD),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EUD} | {error, binary()})
...
-spec start(builder(any(), any(), EUQ)) -> {ok, started(EUQ)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EUX), EUX) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EUZ),
    integer(),
    fun((gleam@erlang@process:subject(EVB)) -> EUZ)
) -> EVB.
