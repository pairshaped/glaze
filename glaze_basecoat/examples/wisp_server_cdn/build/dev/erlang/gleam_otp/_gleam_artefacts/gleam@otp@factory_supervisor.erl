...
-opaque supervisor(FLB, FLC) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(FLB, FLC))}.

-type message(FLD, FLE) :: any() | {gleam_phantom, FLD, FLE}.

-opaque builder(FLF, FLG) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((FLF) -> {ok, gleam@otp@actor:started(FLG)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(FLF, FLG)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(FLH, FLI) :: {local,
        gleam@erlang@process:name(message(FLH, FLI))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(FLJ) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, FLJ}.
...
-type erlang_child_spec_property(FLK, FLL) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((FLK) -> {ok, gleam@otp@actor:started(FLL)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(FLM, FLN))) -> supervisor(FLM, FLN).
get_by_name(Name) ->
...
-spec named(builder(FMD, FME), gleam@erlang@process:name(message(FMD, FME))) -> builder(FMD, FME).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(FMM, FMN), integer(), integer()) -> builder(FMM, FMN).
...
-spec timeout(builder(FMS, FMT), integer()) -> builder(FMS, FMT).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(FMY, FMZ), gleam@otp@supervision:restart()) -> builder(FMY, FMZ).
...
-spec start(builder(FNE, FNF)) -> {ok,
        gleam@otp@actor:started(supervisor(FNE, FNF))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(FOD, FOE)) -> gleam@otp@supervision:child_specification(supervisor(FOD, FOE)).
supervised(Builder) ->
...
-spec start_child(supervisor(FOK, FOL), FOK) -> {ok,
        gleam@otp@actor:started(FOL)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((FPI) -> {ok, gleam@otp@actor:started(FPJ)} |
        {error, gleam@otp@actor:start_error()}),
    FPI
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), FPJ, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((FLT) -> {ok, gleam@otp@actor:started(FLU)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FLT, FLU).
...
-spec supervisor_child(
    fun((FLY) -> {ok, gleam@otp@actor:started(FLZ)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FLY, FLZ).
