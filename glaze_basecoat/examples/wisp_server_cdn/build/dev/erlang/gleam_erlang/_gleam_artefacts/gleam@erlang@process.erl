...
-opaque subject(EDH) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EDH)}.

-type name(EDI) :: any() | {gleam_phantom, EDI}.

-type do_not_leak() :: any().

-type selector(EDJ) :: any() | {gleam_phantom, EDJ}.
...
-spec 'receive'(subject(EEH), integer()) -> {ok, EEH} | {error, nil}.
...
-spec receive_forever(subject(EEP)) -> EEP.
...
-spec selector_receive(selector(EET), integer()) -> {ok, EET} | {error, nil}.
...
-spec selector_receive_forever(selector(EEX)) -> EEX.
...
-spec map_selector(selector(EEZ), fun((EEZ) -> EFB)) -> selector(EFB).
map_selector(A, B) ->
...
-spec merge_selector(selector(EFD), selector(EFD)) -> selector(EFD).
merge_selector(A, B) ->
...
-spec select_map(selector(EFO), subject(EFQ), fun((EFQ) -> EFO)) -> selector(EFO).
...
-spec select(selector(EFK), subject(EFK)) -> selector(EFK).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EFY)
) -> selector(EFY).
...
-spec select_other(selector(EGC), fun((gleam@dynamic:dynamic_()) -> EGC)) -> selector(EGC).
...
-spec deselect(selector(EFT), subject(any())) -> selector(EFT).
...
-spec select_specific_monitor(selector(EGO), monitor(), fun((down()) -> EGO)) -> selector(EGO).
...
-spec select_monitors(selector(EGR), fun((down()) -> EGR)) -> selector(EGR).
...
-spec select_trapped_exits(selector(EFH), fun((exit_message()) -> EFH)) -> selector(EFH).
...
-spec deselect_specific_monitor(selector(EGU), monitor()) -> selector(EGU).
...
-spec send_after(subject(EHQ), integer(), EHQ) -> timer().
...
-spec perform_call(
    subject(EGX),
    fun((subject(EGZ)) -> EGX),
    fun((selector(EGZ)) -> {ok, EGZ} | {error, nil})
) -> EGZ.
...
-spec call(subject(EHE), integer(), fun((subject(EHG)) -> EHE)) -> EHG.
...
-spec call_forever(subject(EHI), fun((subject(EHK)) -> EHI)) -> EHK.
