...
-type action(VDI) :: stop | {continue, VDI, fun(() -> action(VDI))}.

-opaque yielder(VDJ) :: {yielder, fun(() -> action(VDJ))}.

-type step(VDK, VDL) :: {next, VDK, VDL} | done.

-type chunk(VDM, VDN) :: {another_by,
        list(VDM),
        VDN,
        VDM,
        fun(() -> action(VDM))} |
...
-type sized_chunk(VDO) :: {another, list(VDO), fun(() -> action(VDO))} |
    {last, list(VDO)} |
    no_more.
...
-spec unfold_loop(VDW, fun((VDW) -> step(VDX, VDW))) -> fun(() -> action(VDX)).
...
-spec unfold(VDR, fun((VDR) -> step(VDS, VDR))) -> yielder(VDS).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> VEB)) -> yielder(VEB).
repeatedly(F) ->
...
-spec from_list(list(VEF)) -> yielder(VEF).
from_list(List) ->
...
-spec transform(yielder(VEP), VER, fun((VER, VEP) -> step(VES, VER))) -> yielder(VES).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(VEZ)), fun((VFB, VEZ) -> VFB), VFB) -> VFB.
...
-spec fold(yielder(VEW), VEY, fun((VEY, VEW) -> VEY)) -> VEY.
...
-spec to_list(yielder(VFE)) -> list(VFE).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(VFH)) -> step(VFH, yielder(VFH)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(VFP)), integer()) -> fun(() -> action(VFP)).
...
-spec take(yielder(VFM), integer()) -> yielder(VFM).
...
-spec drop_loop(fun(() -> action(VFV)), integer()) -> action(VFV).
...
-spec map(yielder(VFY), fun((VFY) -> VGA)) -> yielder(VGA).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(VGG), yielder(VGI), fun((VGG, VGI) -> VGK)) -> yielder(VGK).
...
-spec append_loop(fun(() -> action(VGW)), fun(() -> action(VGW))) -> action(VGW).
...
-spec append(yielder(VGS), yielder(VGS)) -> yielder(VGS).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(VHE)))) -> action(VHE).
...
-spec flatten(yielder(yielder(VHA))) -> yielder(VHA).
flatten(Yielder) ->
...
-spec concat(list(yielder(VHI))) -> yielder(VHI).
concat(Yielders) ->
...
-spec flat_map(yielder(VHM), fun((VHM) -> yielder(VHO))) -> yielder(VHO).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(VHU)), fun((VHU) -> boolean())) -> action(VHU).
...
-spec filter(yielder(VHR), fun((VHR) -> boolean())) -> yielder(VHR).
...
-spec filter_map(yielder(VHX), fun((VHX) -> {ok, VHZ} | {error, any()})) -> yielder(VHZ).
...
-spec cycle(yielder(VIL)) -> yielder(VIL).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(VIT)), fun((VIT) -> boolean())) -> {ok, VIT} |
    {error, nil}.
...
-spec find(yielder(VIP), fun((VIP) -> boolean())) -> {ok, VIP} | {error, nil}.
...
-spec find_map(yielder(VIX), fun((VIX) -> {ok, VIZ} | {error, any()})) -> {ok,
        VIZ} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(VJQ)), integer()) -> fun(() -> action({VJQ,
    integer()})).
...
-spec index(yielder(VJN)) -> yielder({VJN, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(VJT, fun((VJT) -> VJT)) -> yielder(VJT).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(VJY)), fun((VJY) -> boolean())) -> fun(() -> action(VJY)).
...
-spec take_while(yielder(VJV), fun((VJV) -> boolean())) -> yielder(VJV).
...
-spec drop_while_loop(fun(() -> action(VKE)), fun((VKE) -> boolean())) -> action(VKE).
...
-spec drop_while(yielder(VKB), fun((VKB) -> boolean())) -> yielder(VKB).
...
-spec scan_loop(fun(() -> action(VKL)), fun((VKN, VKL) -> VKN), VKN) -> fun(() -> action(VKN)).
...
-spec scan(yielder(VKH), VKJ, fun((VKJ, VKH) -> VKJ)) -> yielder(VKJ).
...
-spec zip_loop(fun(() -> action(VKU)), fun(() -> action(VKW))) -> fun(() -> action({VKU,
    VKW})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(VKP), yielder(VKR)) -> yielder({VKP, VKR}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(VLJ)), fun((VLJ) -> VLL), VLL, list(VLJ)) -> chunk(VLJ, VLL).
...
-spec chunk_loop(fun(() -> action(VLE)), fun((VLE) -> VLG), VLG, VLE) -> action(list(VLE)).
...
-spec chunk(yielder(VKZ), fun((VKZ) -> any())) -> yielder(list(VKZ)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(VLX)), integer(), list(VLX)) -> sized_chunk(VLX).
...
-spec sized_chunk_loop(fun(() -> action(VLT)), integer()) -> fun(() -> action(list(VLT))).
...
-spec sized_chunk(yielder(VLP), integer()) -> yielder(list(VLP)).
...
-spec intersperse_loop(fun(() -> action(VME)), VME) -> action(VME).
...
-spec intersperse(yielder(VMB), VMB) -> yielder(VMB).
...
-spec any_loop(fun(() -> action(VMJ)), fun((VMJ) -> boolean())) -> boolean().
...
-spec any(yielder(VMH), fun((VMH) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(VMN)), fun((VMN) -> boolean())) -> boolean().
...
-spec all(yielder(VML), fun((VML) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(VND) -> fun((gleam@option:option(list(VND))) -> list(VND)).
...
-spec group_updater(fun((VMV) -> VMW)) -> fun((gleam@dict:dict(VMW, list(VMV)), VMV) -> gleam@dict:dict(VMW, list(VMV))).
group_updater(F) ->
...
-spec group(yielder(VMP), fun((VMP) -> VMR)) -> gleam@dict:dict(VMR, list(VMP)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(VNH), fun((VNH, VNH) -> VNH)) -> {ok, VNH} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(VNL)) -> {ok, VNL} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(VNT) -> yielder(VNT).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(VNZ)), fun(() -> action(VNZ))) -> action(VNZ).
...
-spec interleave(yielder(VNV), yielder(VNV)) -> yielder(VNV).
...
-spec fold_until_loop(
    fun(() -> action(VOH)),
    fun((VOJ, VOH) -> gleam@list:continue_or_stop(VOJ)),
    VOJ
) -> VOJ.
...
-spec fold_until(
    yielder(VOD),
    VOF,
    fun((VOF, VOD) -> gleam@list:continue_or_stop(VOF))
) -> VOF.
...
-spec try_fold(yielder(VOL), VON, fun((VON, VOL) -> {ok, VON} | {error, VOO})) -> {ok,
        VON} |
    {error, VOO}.
...
-spec first(yielder(VPB)) -> {ok, VPB} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(VPF), integer()) -> {ok, VPF} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(VPN), fun((VPN) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(VPQ, fun(() -> yielder(VPQ))) -> yielder(VPQ).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(VPT), VPT) -> yielder(VPT).
