...
-spec group(
    ...
    list(lustre@vdom@vattr:attribute(SKF))
) -> lustre@vdom@vnode:element(SKF).
...
-spec item(
    ...
    binary(),
    boolean(),
    list(lustre@vdom@vattr:attribute(SKJ))
) -> lustre@vdom@vnode:element(SKJ).
item(Name, Value, Label_text, Checked, Attrs) ->
    item_(Name, Value, Label_text, Checked, Attrs).

-file("src/glaze/basecoat/radio_group.gleam", 81).
-spec radio(binary(), binary(), list(lustre@vdom@vattr:attribute(SKR))) -> lustre@vdom@vnode:element(SKR).
