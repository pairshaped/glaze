...
-spec method_override(gleam@http@request:request(AFAJ)) -> gleam@http@request:request(AFAJ).
...
-spec sort_keys(list({binary(), AFCA})) -> list({binary(), AFCA}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, AFAU} | {error, any()}) -> {ok, AFAU} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((AFBI) -> {ok, AFBJ} | {error, any()}),
    binary()
) -> fun((AFBI) -> {ok, AFBJ} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AFBC, bitstring()) -> {ok, AFBC} |
        {error, gleam@http@response:response(body())}),
    AFBC
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AFBC}} |
    {error, gleam@http@response:response(body())}.
