...
-opaque app(NPR, NPS, NPT) :: {app,
        fun((NPR) -> {NPS, lustre@effect:effect(NPT)}),
        fun((NPS, NPT) -> {NPS, lustre@effect:effect(NPT)}),
        fun((NPS) -> lustre@vdom@vnode:element(NPT))}.

-opaque simulation(NPU, NPV) :: {simulation,
        fun((NPU, NPV) -> {NPU, lustre@effect:effect(NPV)}),
        fun((NPU) -> lustre@vdom@vnode:element(NPV)),
        list(event(NPV)),
        NPU,
        lustre@vdom@vnode:element(NPV)}.

-type event(NPW) :: {dispatch, NPW} |
...
-spec simple(
    fun((NPX) -> NPY),
    fun((NPY, NPZ) -> NPY),
    fun((NPY) -> lustre@vdom@vnode:element(NPZ))
) -> app(NPX, NPY, NPZ).
...
-spec application(
    fun((NQE) -> {NQF, lustre@effect:effect(NQG)}),
    fun((NQF, NQG) -> {NQF, lustre@effect:effect(NQG)}),
    fun((NQF) -> lustre@vdom@vnode:element(NQG))
) -> app(NQE, NQF, NQG).
...
-spec start(app(NQN, NQO, NQP), NQN) -> simulation(NQO, NQP).
start(App, Args) ->
...
-spec message(simulation(NQV, NQW), NQW) -> simulation(NQV, NQW).
...
-spec problem(simulation(NSB, NSC), binary(), binary()) -> simulation(NSB, NSC).
...
-spec model(simulation(NSH, any())) -> NSH.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), NSM)) -> lustre@vdom@vnode:element(NSM).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), NSR)) -> list(event(NSR)).
history(Simulation) ->
...
-spec click(simulation(NRI, NRJ), lustre@dev@query:'query'()) -> simulation(NRI, NRJ).
...
-spec input(simulation(NRO, NRP), lustre@dev@query:'query'(), binary()) -> simulation(NRO, NRP).
