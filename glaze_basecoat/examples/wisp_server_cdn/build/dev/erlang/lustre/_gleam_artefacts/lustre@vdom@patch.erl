...
-type patch(JET) :: {patch,
        integer(),
        integer(),
        list(change(JET)),
        list(patch(JET))}.

-type change(JEU) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(JEU)),
        list(lustre@vdom@vattr:attribute(JEU))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(JEU)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(JEU)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(JEV)), list(patch(JEV))) -> patch(JEV).
...
?DOC(false).
-spec add_child(patch(JFY), patch(JFY)) -> patch(JFY).
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(JIP),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JIP)), lustre@vdom@vnode:element(JIP))
) -> gleam@json:json().
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(JIT)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JIT)), lustre@vdom@vnode:element(JIT))
) -> gleam@json:json().
...
-spec change_to_json(
    change(JGF),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JGF)), lustre@vdom@vnode:element(JGF))
) -> gleam@json:json().
...
-spec to_json(
    patch(JGC),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JGC)), lustre@vdom@vnode:element(JGC))
) -> gleam@json:json().
to_json(Patch, Memos) ->
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(JFF)),
    list(lustre@vdom@vattr:attribute(JFF))
) -> change(JFF).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(JFP)) -> change(JFP).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(JFS)), integer()) -> change(JFS).
