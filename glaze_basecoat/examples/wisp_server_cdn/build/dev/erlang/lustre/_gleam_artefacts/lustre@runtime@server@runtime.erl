...
-type state(KSK, KSL) :: {state,
        gleam@erlang@process:subject(message(KSL)),
        gleam@erlang@process:selector(message(KSL)),
        gleam@erlang@process:selector(message(KSL)),
        KSK,
        fun((KSK, KSL) -> {KSK, lustre@effect:effect(KSL)}),
        fun((KSK) -> lustre@vdom@vnode:element(KSL)),
        config(KSL),
        lustre@vdom@vnode:element(KSL),
        lustre@vdom@cache:cache(KSL),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KSL)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(KSL)) -> nil))}.

-type config(KSM) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, KSM} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KSM)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KSM)),
        gleam@option:option(KSM),
        gleam@option:option(KSM)}.

-type message(KSN) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KSN))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KSN))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(KSN)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(KSN)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(KSN))} |
    {effect_dispatched_message, KSN} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, KTW} | {error, nil})),
    binary(),
    binary()
) -> {ok, KTW} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KUD)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(KUJ)),
    lustre@effect:effect(KUJ)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KUN)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(KUN)) -> nil)),
    lustre@runtime@transport:client_message(KUN)
) -> nil.
...
?DOC(false).
-spec loop(state(KTG, KTH), message(KTH)) -> gleam@otp@actor:next(state(KTG, KTH), message(KTH)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(KSR))),
    fun((KSV) -> {KSW, lustre@effect:effect(KSR)}),
    fun((KSW, KSR) -> {KSW, lustre@effect:effect(KSR)}),
    fun((KSW) -> lustre@vdom@vnode:element(KSR)),
    config(KSR),
    KSV
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(KSR)))} |
