...
-type mutable_map(HHG, HHH) :: any() | {gleam_phantom, HHG, HHH}.
...
?DOC(false).
-spec unsafe_get(mutable_map(HHM, HHN), HHM) -> HHN.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(HHU, HHV), HHU, fun(() -> HHV)) -> HHV.
...
?DOC(false).
-spec has_key(mutable_map(HIE, any()), HIE) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(HIM, HIN), HIM, HIN) -> mutable_map(HIM, HIN).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(HIY, HIZ), HIY) -> mutable_map(HIY, HIZ).
delete(Map, Key) ->
    maps:remove(Key, Map).
