...
-type runtime(MRP) :: any() | {gleam_phantom, MRP}.
...
-spec application(
    fun((MSL) -> {MSM, lustre@effect:effect(MSN)}),
    fun((MSM, MSN) -> {MSM, lustre@effect:effect(MSN)}),
    fun((MSM) -> lustre@vdom@vnode:element(MSN))
) -> lustre@runtime@app:app(MSL, MSM, MSN).
...
-spec element(lustre@vdom@vnode:element(MRY)) -> lustre@runtime@app:app(any(), nil, MRY).
element(View) ->
    application(
...
-spec simple(
    fun((MSE) -> MSF),
    fun((MSF, MSG) -> MSF),
    fun((MSF) -> lustre@vdom@vnode:element(MSG))
) -> lustre@runtime@app:app(MSE, MSF, MSG).
...
-spec component(
    fun((MSU) -> {MSV, lustre@effect:effect(MSW)}),
    fun((MSV, MSW) -> {MSV, lustre@effect:effect(MSW)}),
    fun((MSV) -> lustre@vdom@vnode:element(MSW)),
    list(lustre@runtime@app:option(MSW))
) -> lustre@runtime@app:app(MSU, MSV, MSW).
...
-spec named(
    lustre@runtime@app:app(MTF, MTG, MTH),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(MTH))
) -> lustre@runtime@app:app(MTF, MTG, MTH).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(MTZ, any(), MUB), binary(), MTZ) -> {ok,
        runtime(MUB)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec supervised(lustre@runtime@app:app(MUR, any(), MUT), MUR) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MUT))).
...
-spec factory(lustre@runtime@app:app(MVA, any(), MVC)) -> gleam@otp@factory_supervisor:builder(MVA, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MVC))).
factory(App) ->
...
-spec send(runtime(MVR), lustre@runtime@server@runtime:message(MVR)) -> nil.
...
-spec dispatch(MVU) -> lustre@runtime@server@runtime:message(MVU).
dispatch(Message) ->
...
-spec start(lustre@runtime@app:app(MTQ, any(), MTS), binary(), MTQ) -> {ok,
        runtime(MTS)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(MUI, any(), MUK), MUI) -> {ok,
        runtime(MUK)} |
    {error, error()}.
