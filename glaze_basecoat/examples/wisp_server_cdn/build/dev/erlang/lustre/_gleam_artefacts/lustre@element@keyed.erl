...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(OAS)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OAS)),
    list(lustre@vdom@vnode:element(OAS))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OAS)),
    list(lustre@vdom@vnode:element(OAS))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(OAK)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OAK)),
    list(lustre@vdom@vnode:element(OAK))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(NYQ)),
    list({binary(), lustre@vdom@vnode:element(NYQ)})
) -> lustre@vdom@vnode:element(NYQ).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(NYW)),
    list({binary(), lustre@vdom@vnode:element(NYW)})
) -> lustre@vdom@vnode:element(NYW).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(NZC)})) -> lustre@vdom@vnode:element(NZC).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(NZG)),
    list({binary(), lustre@vdom@vnode:element(NZG)})
) -> lustre@vdom@vnode:element(NZG).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(NZM)),
    list({binary(), lustre@vdom@vnode:element(NZM)})
) -> lustre@vdom@vnode:element(NZM).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(NZS)),
    list({binary(), lustre@vdom@vnode:element(NZS)})
) -> lustre@vdom@vnode:element(NZS).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(NZY)),
    list({binary(), lustre@vdom@vnode:element(NZY)})
) -> lustre@vdom@vnode:element(NZY).
...
-spec dl(
    ...
    list({binary(), lustre@vdom@vnode:element(OAE)})
) -> lustre@vdom@vnode:element(OAE).
