...
-opaque effect(HAX) :: {effect,
        list(fun((actions(HAX)) -> nil)),
        list(fun((actions(HAX)) -> nil)),
        list(fun((actions(HAX)) -> nil))}.

-type actions(HAY) :: {actions,
        fun((HAY) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(HAY)) -> nil),
...
-spec do_comap_select(
    actions(HCI),
    gleam@erlang@process:selector(HCK),
    fun((HCK) -> HCI)
) -> nil.
...
-spec do_comap_actions(actions(HCE), fun((HCG) -> HCE)) -> actions(HCG).
...
-spec do_map(list(fun((actions(HBY)) -> nil)), fun((HBY) -> HCB)) -> list(fun((actions(HCB)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(HBU), fun((HBU) -> HBW)) -> effect(HBW).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((HCM) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(HCM)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((HBB) -> nil)) -> nil)) -> effect(HBB).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((HBD) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(HBD).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((HBF) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(HBF).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((HBJ) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(HBJ))
) -> effect(HBJ).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(HBQ))) -> effect(HBQ).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
