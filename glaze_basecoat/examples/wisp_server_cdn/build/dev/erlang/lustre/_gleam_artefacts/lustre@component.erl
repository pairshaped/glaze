...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, MDS} | {error, nil})) -> lustre@runtime@app:option(MDS).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(MDW)) -> lustre@runtime@app:option(MDW).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(MDZ)) -> lustre@runtime@app:option(MDZ).
...
-spec on_form_autofill(fun((binary()) -> MEE)) -> lustre@runtime@app:option(MEE).
...
-spec on_form_reset(MEG) -> lustre@runtime@app:option(MEG).
...
-spec on_form_restore(fun((binary()) -> MEI)) -> lustre@runtime@app:option(MEI).
...
-spec on_connect(MEQ) -> lustre@runtime@app:option(MEQ).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(MES) -> lustre@runtime@app:option(MES).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(MEU) -> lustre@runtime@app:option(MEU).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(MEW)),
    list(lustre@vdom@vnode:element(MEW))
) -> lustre@vdom@vnode:element(MEW).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(MFC)),
    list(lustre@vdom@vnode:element(MFC))
) -> lustre@vdom@vnode:element(MFC).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), MGC),
    binary(),
    list(lustre@vdom@vattr:attribute(MGC)),
    list(lustre@vdom@vnode:element(MGC))
) -> lustre@vdom@vnode:element(MGC).
