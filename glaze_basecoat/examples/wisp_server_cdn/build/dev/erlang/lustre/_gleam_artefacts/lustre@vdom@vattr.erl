...
-type attribute(GBV) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(GBV)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(GBW) :: {handler, boolean(), boolean(), GBW}.
...
?DOC(false).
-spec merge(list(attribute(GCL)), list(attribute(GCL))) -> list(attribute(GCL)).
...
?DOC(false).
-spec compare(attribute(GCS), attribute(GCS)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(GCG))) -> list(attribute(GCG)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(GCB)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
