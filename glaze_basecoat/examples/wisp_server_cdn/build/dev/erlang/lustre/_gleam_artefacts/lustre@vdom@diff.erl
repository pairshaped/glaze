...
-type diff(KGO) :: {diff,
        lustre@vdom@patch:patch(KGO),
        lustre@vdom@cache:cache(KGO)}.

-type partial_diff(KGP) :: {partial_diff,
        lustre@vdom@patch:patch(KGP),
        lustre@vdom@cache:cache(KGP),
        lustre@vdom@cache:events(KGP)}.

-type attribute_change(KGQ) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(KGQ)),
        list(lustre@vdom@vattr:attribute(KGQ)),
        lustre@vdom@cache:events(KGQ)}.
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(KGW)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(KGW)),
    list(lustre@vdom@vnode:element(KGW)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(KGW)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(KGW)),
    list(lustre@vdom@patch:patch(KGW)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(KGW),
    lustre@vdom@cache:events(KGW)
) -> partial_diff(KGW).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(KGR),
    lustre@vdom@vnode:element(KGR),
    lustre@vdom@vnode:element(KGR)
) -> diff(KGR).
diff(Cache, Old, New) ->
