...
-type message(XVG) :: {packet, bitstring()} | {user, XVG}.
...
-type connection(XVH) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(XVH))}.

-opaque next(XVI, XVJ) :: {continue,
        XVI,
        gleam@option:option(gleam@erlang@process:selector(XVJ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(XVK, XVL) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(XVL)) -> {XVK,
            gleam@option:option(gleam@erlang@process:selector(XVL))}),
        fun((XVK, message(XVL), connection(XVL)) -> next(XVK, message(XVL))),
        gleam@option:option(fun((XVK) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(XWL) -> next(XWL, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(XWP, XWQ), gleam@erlang@process:selector(XWQ)) -> next(XWP, XWQ).
...
?DOC(false).
-spec convert_next(next(XXE, XXF)) -> glisten@internal@handler:next(XXE, XXF).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(XXK, XXL), fun((XXL) -> XXO)) -> next(XXK, XXO).
...
-spec map_user_selector(gleam@erlang@process:selector(message(XXR))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(XXR)).
...
-spec convert_loop(
    fun((XXW, message(XXX), connection(XXX)) -> next(XXW, message(XXX)))
) -> fun((XXW, glisten@internal@handler:loop_message(XXX), glisten@internal@handler:connection(XXX)) -> glisten@internal@handler:next(XXW, glisten@internal@handler:loop_message(XXX))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(XYC)) -> {XYE,
        gleam@option:option(gleam@erlang@process:selector(XYC))})
) -> fun((glisten@internal@handler:connection(XYC)) -> {XYE,
    gleam@option:option(gleam@erlang@process:selector(XYC))}).
...
-spec new(
    fun((connection(XYK)) -> {XYM,
        gleam@option:option(gleam@erlang@process:selector(XYK))}),
    fun((XYM, message(XYK), connection(XYK)) -> next(XYM, message(XYK)))
) -> builder(XYM, XYK).
new(On_init, Loop) ->
...
-spec with_close(builder(XYT, XYU), fun((XYT) -> nil)) -> builder(XYT, XYU).
...
-spec with_pool_size(builder(XYZ, XZA), integer()) -> builder(XYZ, XZA).
...
?DOC(false).
-spec with_http2(builder(XZF, XZG)) -> builder(XZF, XZG).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(XZR, XZS)) -> builder(XZR, XZS).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(XZX, XZY), binary(), binary()) -> builder(XZX, XZY).
...
-spec bind(builder(XZL, XZM), binary()) -> builder(XZL, XZM).
