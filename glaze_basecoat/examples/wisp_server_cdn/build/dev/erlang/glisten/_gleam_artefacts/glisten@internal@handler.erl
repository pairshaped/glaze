...
-type message(XBF) :: {internal, internal_message()} | {user, XBF}.

-type loop_message(XBG) :: {packet, bitstring()} | {custom, XBG}.

-type loop_state(XBH, XBI) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(XBI)),
        glisten@transport:transport(),
        XBH}.

-type connection(XBJ) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(XBJ))}.

-type next(XBK, XBL) :: {continue,
        XBK,
        gleam@option:option(gleam@erlang@process:selector(XBL))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(XBM, XBN) :: {handler,
        glisten@socket:socket(),
        fun((XBM, loop_message(XBN), connection(XBN)) -> next(XBM, loop_message(XBN))),
        fun((connection(XBN)) -> {XBM,
            gleam@option:option(gleam@erlang@process:selector(XBN))}),
        gleam@option:option(fun((XBM) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(XCA) -> next(XCA, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(XCE, XCF), gleam@erlang@process:selector(XCF)) -> next(XCE, XCF).
...
?DOC(false).
-spec start(handler(any(), XCU)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(XCU)))} |
