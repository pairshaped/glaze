...
-spec be_ok({ok, WLV} | {error, any()}) -> WLV.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, WMA}) -> WMA.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(WMD)) -> WMD.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
