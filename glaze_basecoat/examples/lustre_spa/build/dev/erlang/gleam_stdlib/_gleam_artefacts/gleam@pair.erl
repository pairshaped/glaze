...
-spec map_first({CNP, CNQ}, fun((CNP) -> CNR)) -> {CNR, CNQ}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CNS, CNT}, fun((CNT) -> CNU)) -> {CNS, CNU}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
