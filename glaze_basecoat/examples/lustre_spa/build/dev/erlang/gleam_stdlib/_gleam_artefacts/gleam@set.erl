...
-opaque set(CWV) :: {set, gleam@dict:dict(CWV, list(nil))}.
...
-spec contains(set(CXG), CXG) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CXI), CXI) -> set(CXI).
delete(Set, Member) ->
...
-spec fold(set(CXR), CXT, fun((CXT, CXR) -> CXT)) -> CXT.
...
-spec filter(set(CXU), fun((CXU) -> boolean())) -> set(CXU).
...
-spec drop(set(CYB), list(CYB)) -> set(CYB).
drop(Set, Disallowed) ->
...
-spec take(set(CYF), list(CYF)) -> set(CYF).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CYN), set(CYN)) -> {set(CYN), set(CYN)}.
order(First, Second) ->
...
-spec intersection(set(CYS), set(CYS)) -> set(CYS).
...
-spec difference(set(CYW), set(CYW)) -> set(CYW).
...
-spec is_subset(set(CZA), set(CZA)) -> boolean().
...
-spec is_disjoint(set(CZD), set(CZD)) -> boolean().
...
-spec each(set(CZK), fun((CZK) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CXD), CXD) -> set(CXD).
insert(Set, Member) ->
...
-spec from_list(list(CXO)) -> set(CXO).
from_list(Members) ->
...
-spec map(set(CXX), fun((CXX) -> CXZ)) -> set(CXZ).
map(Set, Fun) ->
...
-spec union(set(CYJ), set(CYJ)) -> set(CYJ).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CZG), set(CZG)) -> set(CZG).
