...
-spec map({ok, COG} | {error, COH}, fun((COG) -> COK)) -> {ok, COK} |
    {error, COH}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CON} | {error, COO}, fun((COO) -> COR)) -> {ok, CON} |
    {error, COR}.
...
-spec flatten({ok, {ok, COU} | {error, COV}} | {error, COV}) -> {ok, COU} |
    {error, COV}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CPC} | {error, CPD}, fun((CPC) -> {ok, CPG} | {error, CPD})) -> {ok,
        CPG} |
    {error, CPD}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, CPL} | {error, any()}, CPL) -> CPL.
...
-spec lazy_unwrap({ok, CPP} | {error, any()}, fun(() -> CPP)) -> CPP.
...
-spec unwrap_error({ok, any()} | {error, CPU}, CPU) -> CPU.
...
-spec 'or'({ok, CPX} | {error, CPY}, {ok, CPX} | {error, CPY}) -> {ok, CPX} |
    {error, CPY}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CQF} | {error, CQG}, fun(() -> {ok, CQF} | {error, CQG})) -> {ok,
        CQF} |
    {error, CQG}.
...
-spec all(list({ok, CQN} | {error, CQO})) -> {ok, list(CQN)} | {error, CQO}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 307).
-spec partition_loop(list({ok, CRC} | {error, CRD}), list(CRC), list(CRD)) -> {list(CRC),
    list(CRD)}.
...
-spec partition(list({ok, CQV} | {error, CQW})) -> {list(CQV), list(CQW)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CRL}, CRO) -> {ok, CRO} | {error, CRL}.
...
-spec replace_error({ok, CRR} | {error, any()}, CRV) -> {ok, CRR} | {error, CRV}.
...
-spec values(list({ok, CRY} | {error, any()})) -> list(CRY).
values(Results) ->
