...
-type group_registry(ACWU) :: any() | {gleam_phantom, ACWU}.

-type message(ACWV) :: any() | {gleam_phantom, ACWV}.

-type do_not_leak() :: any().

-file("src/group_registry.gleam", 104).
-spec get_registry(gleam@erlang@process:name(message(ACXG))) -> group_registry(ACXG).
get_registry(Name) ->
...
-spec start(gleam@erlang@process:name(message(ACWW))) -> {ok,
        gleam@otp@actor:started(group_registry(ACWW))} |
...
-spec supervised(gleam@erlang@process:name(message(ACXB))) -> gleam@otp@supervision:child_specification(group_registry(ACXB)).
supervised(Name) ->
...
-spec make_subject(group_registry(ACXU), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(ACXU).
...
-spec join(group_registry(ACXK), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(ACXK).
...
-spec members(group_registry(ACXQ), binary()) -> list(gleam@erlang@process:subject(ACXQ)).
