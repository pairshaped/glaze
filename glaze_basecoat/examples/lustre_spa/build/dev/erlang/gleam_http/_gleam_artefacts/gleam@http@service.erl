...
-spec map_response_body(
    fun((VOJ) -> gleam@http@response:response(VOK)),
    fun((VOK) -> VOM)
) -> fun((VOJ) -> gleam@http@response:response(VOM)).
...
-spec prepend_response_header(
    fun((VOO) -> gleam@http@response:response(VOP)),
    binary(),
    binary()
) -> fun((VOO) -> gleam@http@response:response(VOP)).
...
-spec ensure_post(gleam@http@request:request(VOS)) -> {ok,
        gleam@http@request:request(VOS)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(VPB)) -> VPD)) -> fun((gleam@http@request:request(VPB)) -> VPD).
