...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, MEV} | {error, nil})) -> lustre@runtime@app:option(MEV).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(MEZ)) -> lustre@runtime@app:option(MEZ).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(MFC)) -> lustre@runtime@app:option(MFC).
...
-spec on_form_autofill(fun((binary()) -> MFH)) -> lustre@runtime@app:option(MFH).
...
-spec on_form_reset(MFJ) -> lustre@runtime@app:option(MFJ).
...
-spec on_form_restore(fun((binary()) -> MFL)) -> lustre@runtime@app:option(MFL).
...
-spec on_connect(MFT) -> lustre@runtime@app:option(MFT).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(MFV) -> lustre@runtime@app:option(MFV).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(MFX) -> lustre@runtime@app:option(MFX).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(MFZ)),
    list(lustre@vdom@vnode:element(MFZ))
) -> lustre@vdom@vnode:element(MFZ).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(MGF)),
    list(lustre@vdom@vnode:element(MGF))
) -> lustre@vdom@vnode:element(MGF).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), MHF),
    binary(),
    list(lustre@vdom@vattr:attribute(MHF)),
    list(lustre@vdom@vnode:element(MHF))
) -> lustre@vdom@vnode:element(MHF).
