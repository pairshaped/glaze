...
-spec include(lustre@vdom@vattr:attribute(PJP), list(binary())) -> lustre@vdom@vattr:attribute(PJP).
...
-spec subject(lustre:runtime(PJT)) -> gleam@erlang@process:subject(lustre@runtime@server@runtime:message(PJT)).
subject(Runtime) ->
...
-spec register_subject(
    gleam@erlang@process:subject(lustre@runtime@transport:client_message(PKB))
) -> lustre@runtime@server@runtime:message(PKB).
...
-spec deregister_subject(
    gleam@erlang@process:subject(lustre@runtime@transport:client_message(PKF))
) -> lustre@runtime@server@runtime:message(PKF).
...
-spec register_callback(
    fun((lustre@runtime@transport:client_message(PKJ)) -> nil)
) -> lustre@runtime@server@runtime:message(PKJ).
...
-spec deregister_callback(
    fun((lustre@runtime@transport:client_message(PKM)) -> nil)
) -> lustre@runtime@server@runtime:message(PKM).
...
-spec select(
    fun((fun((PKR) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(PKR))
) -> lustre@effect:effect(PKR).
select(Sel) ->
