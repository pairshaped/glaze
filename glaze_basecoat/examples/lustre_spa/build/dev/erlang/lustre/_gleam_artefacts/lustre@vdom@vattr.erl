...
-type attribute(GCY) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(GCY)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(GCZ) :: {handler, boolean(), boolean(), GCZ}.
...
?DOC(false).
-spec merge(list(attribute(GDO)), list(attribute(GDO))) -> list(attribute(GDO)).
...
?DOC(false).
-spec compare(attribute(GDV), attribute(GDV)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(GDJ))) -> list(attribute(GDJ)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(GDE)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
