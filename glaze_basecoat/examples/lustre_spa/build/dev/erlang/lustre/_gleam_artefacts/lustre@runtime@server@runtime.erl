...
-type state(KTN, KTO) :: {state,
        gleam@erlang@process:subject(message(KTO)),
        gleam@erlang@process:selector(message(KTO)),
        gleam@erlang@process:selector(message(KTO)),
        KTN,
        fun((KTN, KTO) -> {KTN, lustre@effect:effect(KTO)}),
        fun((KTN) -> lustre@vdom@vnode:element(KTO)),
        config(KTO),
        lustre@vdom@vnode:element(KTO),
        lustre@vdom@cache:cache(KTO),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KTO)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(KTO)) -> nil))}.

-type config(KTP) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, KTP} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KTP)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KTP)),
        gleam@option:option(KTP),
        gleam@option:option(KTP)}.

-type message(KTQ) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KTQ))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KTQ))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(KTQ)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(KTQ)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(KTQ))} |
    {effect_dispatched_message, KTQ} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, KUZ} | {error, nil})),
    binary(),
    binary()
) -> {ok, KUZ} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KVG)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(KVM)),
    lustre@effect:effect(KVM)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KVQ)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(KVQ)) -> nil)),
    lustre@runtime@transport:client_message(KVQ)
) -> nil.
...
?DOC(false).
-spec loop(state(KUJ, KUK), message(KUK)) -> gleam@otp@actor:next(state(KUJ, KUK), message(KUK)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(KTU))),
    fun((KTY) -> {KTZ, lustre@effect:effect(KTU)}),
    fun((KTZ, KTU) -> {KTZ, lustre@effect:effect(KTU)}),
    fun((KTZ) -> lustre@vdom@vnode:element(KTU)),
    config(KTU),
    KTY
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(KTU)))} |
