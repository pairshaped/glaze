...
-spec on(binary(), gleam@dynamic@decode:decoder(PAU)) -> lustre@vdom@vattr:attribute(PAU).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PAX))
) -> lustre@vdom@vattr:attribute(PAX).
...
-spec handler(PBB, boolean(), boolean()) -> lustre@vdom@vattr:handler(PBB).
...
-spec prevent_default(lustre@vdom@vattr:attribute(PBD)) -> lustre@vdom@vattr:attribute(PBD).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(PBG)) -> lustre@vdom@vattr:attribute(PBG).
...
-spec debounce(lustre@vdom@vattr:attribute(PBJ), integer()) -> lustre@vdom@vattr:attribute(PBJ).
...
-spec throttle(lustre@vdom@vattr:attribute(PBM), integer()) -> lustre@vdom@vattr:attribute(PBM).
...
-spec on_keypress(fun((binary()) -> PCD)) -> lustre@vdom@vattr:attribute(PCD).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> PCF)) -> lustre@vdom@vattr:attribute(PCF).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> PCH)) -> lustre@vdom@vattr:attribute(PCH).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> PCJ)) -> lustre@vdom@vattr:attribute(PCJ).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> PCL)) -> lustre@vdom@vattr:attribute(PCL).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> PCN)) -> lustre@vdom@vattr:attribute(PCN).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> PCQ)) -> lustre@vdom@vattr:attribute(PCQ).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(PCU) -> lustre@vdom@vattr:attribute(PCU).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(PCW) -> lustre@vdom@vattr:attribute(PCW).
on_blur(Msg) ->
