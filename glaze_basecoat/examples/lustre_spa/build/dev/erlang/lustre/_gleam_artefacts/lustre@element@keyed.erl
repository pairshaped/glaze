...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(OBV)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OBV)),
    list(lustre@vdom@vnode:element(OBV))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OBV)),
    list(lustre@vdom@vnode:element(OBV))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(OBN)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(OBN)),
    list(lustre@vdom@vnode:element(OBN))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(NZT)),
    list({binary(), lustre@vdom@vnode:element(NZT)})
) -> lustre@vdom@vnode:element(NZT).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(NZZ)),
    list({binary(), lustre@vdom@vnode:element(NZZ)})
) -> lustre@vdom@vnode:element(NZZ).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(OAF)})) -> lustre@vdom@vnode:element(OAF).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(OAJ)),
    list({binary(), lustre@vdom@vnode:element(OAJ)})
) -> lustre@vdom@vnode:element(OAJ).
...
-spec ol(
    ...
    list({binary(), lustre@vdom@vnode:element(OAP)})
) -> lustre@vdom@vnode:element(OAP).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(OAV)),
    list({binary(), lustre@vdom@vnode:element(OAV)})
) -> lustre@vdom@vnode:element(OAV).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(OBB)),
    list({binary(), lustre@vdom@vnode:element(OBB)})
) -> lustre@vdom@vnode:element(OBB).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(OBH)),
    list({binary(), lustre@vdom@vnode:element(OBH)})
) -> lustre@vdom@vnode:element(OBH).
