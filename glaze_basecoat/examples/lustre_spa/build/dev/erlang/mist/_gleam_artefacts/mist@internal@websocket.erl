...
-type valid_message(AFDY) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, AFDY}.

-type websocket_message(AFDZ) :: {valid, valid_message(AFDZ)} | invalid.
...
-type handler_message(AFEA) :: {internal, gramps@websocket:frame()} |
    {user, AFEA}.

-type websocket_state(AFEB) :: {websocket_state,
        bitstring(),
        AFEB,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(AFFH))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(AFFH))).
...
-spec apply_frames(
    ...
    fun((AFEX, handler_message(AFEY), websocket_connection()) -> mist@internal@next:next(AFEX, AFEY)),
    websocket_connection(),
    mist@internal@next:next(AFEX, websocket_message(AFEY)),
    fun((AFEX) -> nil)
) -> mist@internal@next:next(AFEX, websocket_message(AFEY)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {AFEK,
        gleam@option:option(gleam@erlang@process:selector(AFEL))}),
    fun((AFEK) -> nil),
    fun((AFEK, handler_message(AFEL), websocket_connection()) -> mist@internal@next:next(AFEK, AFEL)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(AFEL)))} |
    {error, nil}.
