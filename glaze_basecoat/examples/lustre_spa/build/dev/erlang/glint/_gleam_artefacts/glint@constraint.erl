...
-define(FILEPATH, "src/glint/constraint.gleam").
...
-spec one_of(list(XUO)) -> fun((XUO) -> {ok, XUO} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(XUR)) -> fun((XUR) -> {ok, XUR} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((XUU) -> {ok, XUU} | {error, snag:snag()})) -> fun((list(XUU)) -> {ok,
        list(XUU)} |
    {error, snag:snag()}).
each(Constraint) ->
