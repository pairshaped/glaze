...
-opaque glint(YAY) :: {glint, config(), command_node(YAY)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(YAZ) :: {command,
        fun((named_args(), list(binary()), flags()) -> YAZ),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(YBA) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> YBA),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(YBB) :: {command_node,
        gleam@option:option(internal_command(YBB)),
        gleam@dict:dict(binary(), command_node(YBB)),
        flags(),
        binary()}.

-type out(YBC) :: {out, YBC} | {help, binary()}.

-type value() :: {b, flag_internals(boolean())} |
    {i, flag_internals(integer())} |
    {l_i, flag_internals(list(integer()))} |
    {f, flag_internals(float())} |
    {l_f, flag_internals(list(float()))} |
    {s, flag_internals(binary())} |
    {l_s, flag_internals(list(binary()))}.

-opaque flag(YBD) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, YBD} | {error, snag:snag()}),
        fun((flag_internals(YBD)) -> value()),
        fun((flags(), binary()) -> {ok, YBD} | {error, snag:snag()}),
        gleam@option:option(YBD)}.

-type flag_internals(YBE) :: {flag_internals,
        gleam@option:option(YBE),
        fun((binary()) -> {ok, YBE} | {error, snag:snag()})}.
...
-spec pretty_help(glint(YBL), pretty_help()) -> glint(YBL).
...
-spec with_name(glint(YBO), binary()) -> glint(YBO).
...
-spec without_exit(glint(YBR)) -> glint(YBR).
without_exit(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec as_module(glint(YBU)) -> glint(YBU).
as_module(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec with_indent_width(glint(YBX), integer()) -> glint(YBX).
...
-spec with_max_output_width(glint(YCA), integer()) -> glint(YCA).
...
-spec with_min_first_column_width(glint(YCD), integer()) -> glint(YCD).
...
-spec with_column_gap(glint(YCG), integer()) -> glint(YCG).
...
-spec global_help(glint(YCP), binary()) -> glint(YCP).
...
-spec map_command(command(YDE), fun((YDE) -> YDG)) -> command(YDG).
...
-spec command_help(binary(), fun(() -> command(YDI))) -> command(YDI).
command_help(Desc, F) ->
    _record = F(),
    {command,
        erlang:element(2, _record),
...
-spec unnamed_args(args_count(), fun(() -> command(YDL))) -> command(YDL).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(YDO))
) -> command(YDO).
named_arg(Name, F) ->
    Cmd = begin
        F(
            fun(Named_args) ->
                Arg@1 = case gleam_stdlib:map_get(
...
-spec new_builder(
    binary(),
    fun((flag_internals(YGC)) -> value()),
    fun((flags(), binary()) -> {ok, YGC} | {error, snag:snag()}),
    fun((binary()) -> {ok, YGC} | {error, snag:snag()})
) -> flag(YGC).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, YGO} | {error, snag:snag()}),
    fun((YGO) -> {ok, YGO} | {error, snag:snag()})
) -> fun((binary()) -> {ok, YGO} | {error, snag:snag()}).
...
-spec flag_constraint(flag(YGK), fun((YGK) -> {ok, YGK} | {error, snag:snag()})) -> flag(YGK).
...
-spec flag_help(flag(YHD), binary()) -> flag(YHD).
flag_help(Flag, Description) ->
    {flag,
        erlang:element(2, Flag),
        Description,
        erlang:element(4, Flag),
        erlang:element(5, Flag),
        erlang:element(6, Flag),
        erlang:element(7, Flag)}.
...
-spec flag_default(flag(YHG), YHG) -> flag(YHG).
...
-spec flag(
    flag(YDR),
    fun((fun((flags()) -> {ok, YDR} | {error, snag:snag()})) -> command(YDU))
) -> command(YDU).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), YHJ, fun((YHJ, binary(), flag_entry()) -> YHJ)) -> YHJ.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> YDB)) -> command(YDB).
command(Runner) ->
...
construct_value(Input, Internal, Constructor) ->
    gleam@result:map(
        ...
        fun(Val) ->
            Constructor(
                {flag_internals, {some, Val}, erlang:element(3, Internal)}
            )
        end
    ).
            ...
            gleam@result:map(
                ...
                fun(Value) ->
                    insert(
                        Flags,
                        Key,
                        {flag_entry, Value, erlang:element(3, Contents)}
                    )
                end
            )
        end
    ...
    gleam@result:'try'(
        ...
        fun(Contents) -> case erlang:element(2, Contents) of
                {b, {flag_internals, none, _} = Internal} ->
                    _pipe = {flag_internals,
                        {some, true},
                        erlang:element(3, Internal)},
                    _pipe@1 = {b, _pipe},
                    _pipe@2 = (fun(Val) ->
                        {flag_entry, Val, erlang:element(3, Contents)}
                    end)(_pipe@1),
                ...
                {b, {flag_internals, {some, Val@1}, _} = Internal@1} ->
                    _pipe@5 = {flag_internals,
                        {some, not Val@1},
                        erlang:element(3, Internal@1)},
                    _pipe@6 = {b, _pipe@5},
                    _pipe@7 = (fun(Val@2) ->
                        {flag_entry, Val@2, erlang:element(3, Contents)}
                    end)(_pipe@6),
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, YHW} | {error, snag:snag()})
) -> {ok, YHW} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(YHZ)) -> {ok, YHZ} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
get_ints_flag(Flags, Name) ->
    get_value(Flags, Name, fun(Flag) -> case erlang:element(2, Flag) of
                {l_i, {flag_internals, {some, Val}, _}} ->
                    {ok, Val};

                {l_i, {flag_internals, none, _}} ->
                    flag_not_provided_error();
...
get_strings_flag(Flags, Name) ->
    get_value(Flags, Name, fun(Flag) -> case erlang:element(2, Flag) of
                {l_s, {flag_internals, {some, Val}, _}} ->
                    {ok, Val};

                {l_s, {flag_internals, none, _}} ->
                    flag_not_provided_error();
...
get_floats_flag(Flags, Name) ->
    get_value(Flags, Name, fun(Flag) -> case erlang:element(2, Flag) of
                {l_f, {flag_internals, {some, Val}, _}} ->
                    {ok, Val};

                {l_f, {flag_internals, none, _}} ->
                    flag_not_provided_error();
...
-spec do_update_at(
    command_node(YIS),
    list(binary()),
    fun((command_node(YIS)) -> command_node(YIS))
) -> command_node(YIS).
...
-spec update_at(
    glint(YIM),
    list(binary()),
    fun((command_node(YIM)) -> command_node(YIM))
) -> glint(YIM).
...
-spec path_help(glint(YCL), list(binary()), binary()) -> glint(YCL).
...
-spec add(glint(YCS), list(binary()), command(YCS)) -> glint(YCS).
...
-spec group_flag(glint(YDX), list(binary()), flag(any())) -> glint(YDX).
...
?DOC(false).
-spec execute(glint(YED), list(binary())) -> {ok, out(YED)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(YFC), list(binary()), fun((YFC) -> any())) -> nil.
