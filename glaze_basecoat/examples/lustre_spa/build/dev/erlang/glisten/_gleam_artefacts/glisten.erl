...
-type message(ABPB) :: {packet, bitstring()} | {user, ABPB}.
...
-type connection(ABPC) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(ABPC))}.

-opaque next(ABPD, ABPE) :: {continue,
        ABPD,
        gleam@option:option(gleam@erlang@process:selector(ABPE))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(ABPF, ABPG) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(ABPG)) -> {ABPF,
            gleam@option:option(gleam@erlang@process:selector(ABPG))}),
        fun((ABPF, message(ABPG), connection(ABPG)) -> next(ABPF, message(ABPG))),
        gleam@option:option(fun((ABPF) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(ABQG) -> next(ABQG, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(ABQK, ABQL), gleam@erlang@process:selector(ABQL)) -> next(ABQK, ABQL).
...
?DOC(false).
-spec convert_next(next(ABQZ, ABRA)) -> glisten@internal@handler:next(ABQZ, ABRA).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(ABRF, ABRG), fun((ABRG) -> ABRJ)) -> next(ABRF, ABRJ).
...
-spec map_user_selector(gleam@erlang@process:selector(message(ABRM))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(ABRM)).
...
-spec convert_loop(
    fun((ABRR, message(ABRS), connection(ABRS)) -> next(ABRR, message(ABRS)))
) -> fun((ABRR, glisten@internal@handler:loop_message(ABRS), glisten@internal@handler:connection(ABRS)) -> glisten@internal@handler:next(ABRR, glisten@internal@handler:loop_message(ABRS))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(ABRX)) -> {ABRZ,
        gleam@option:option(gleam@erlang@process:selector(ABRX))})
) -> fun((glisten@internal@handler:connection(ABRX)) -> {ABRZ,
    gleam@option:option(gleam@erlang@process:selector(ABRX))}).
...
-spec new(
    fun((connection(ABSF)) -> {ABSH,
        gleam@option:option(gleam@erlang@process:selector(ABSF))}),
    fun((ABSH, message(ABSF), connection(ABSF)) -> next(ABSH, message(ABSF)))
) -> builder(ABSH, ABSF).
new(On_init, Loop) ->
...
-spec with_close(builder(ABSO, ABSP), fun((ABSO) -> nil)) -> builder(ABSO, ABSP).
...
-spec with_pool_size(builder(ABSU, ABSV), integer()) -> builder(ABSU, ABSV).
...
?DOC(false).
-spec with_http2(builder(ABTA, ABTB)) -> builder(ABTA, ABTB).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(ABTM, ABTN)) -> builder(ABTM, ABTN).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ABTS, ABTT), binary(), binary()) -> builder(ABTS, ABTT).
...
-spec bind(builder(ABTG, ABTH), binary()) -> builder(ABTG, ABTH).
