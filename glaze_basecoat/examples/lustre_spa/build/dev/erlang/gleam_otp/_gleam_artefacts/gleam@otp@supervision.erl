...
-type child_specification(FJQ) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FJQ)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FJR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FJR).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FJW)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FJW).
supervisor(Start) ->
...
-spec significant(child_specification(FKB), boolean()) -> child_specification(FKB).
...
-spec timeout(child_specification(FKE), integer()) -> child_specification(FKE).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FKH), restart()) -> child_specification(FKH).
...
-spec map_data(child_specification(FKK), fun((FKK) -> FKM)) -> child_specification(FKM).
