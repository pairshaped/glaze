...
-type message(EQL) :: {message, EQL} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EQM, EQN) :: {continue,
        EQM,
        gleam@option:option(gleam@erlang@process:selector(EQN))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EQO, EQP) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EQO,
        gleam@erlang@process:selector(message(EQP)),
        gleam@otp@system:debug_state(),
        fun((EQO, EQP) -> next(EQO, EQP))}.

-type started(EQQ) :: {started, gleam@erlang@process:pid_(), EQQ}.

-opaque initialised(EQR, EQS, EQT) :: {initialised,
        EQR,
        gleam@option:option(gleam@erlang@process:selector(EQS)),
        EQT}.

-opaque builder(EQU, EQV, EQW) :: {builder,
        fun((gleam@erlang@process:subject(EQV)) -> {ok,
                initialised(EQU, EQV, EQW)} |
            {error, binary()}),
        integer(),
        fun((EQU, EQV) -> next(EQU, EQV)),
        gleam@option:option(gleam@erlang@process:name(EQV))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EQX) :: {ack, {ok, EQX} | {error, binary()}} |
...
-spec continue(ERC) -> next(ERC, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(ERO, ERP), gleam@erlang@process:selector(ERP)) -> next(ERO, ERP).
...
-spec initialised(ERV) -> initialised(ERV, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(ESA, any(), ESC),
    gleam@erlang@process:selector(ESG)
) -> initialised(ESA, ESG, ESC).
...
-spec returning(initialised(ESL, ESM, any()), ESR) -> initialised(ESL, ESM, ESR).
...
-spec new(ESV) -> builder(ESV, ESW, gleam@erlang@process:subject(ESW)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(ETB)) -> {ok, initialised(ETD, ETB, ETE)} |
        {error, binary()})
) -> builder(ETD, ETB, ETE).
...
-spec on_message(builder(ETN, ETO, ETP), fun((ETN, ETO) -> next(ETN, ETO))) -> builder(ETN, ETO, ETP).
...
-spec named(builder(ETY, ETZ, EUA), gleam@erlang@process:name(ETZ)) -> builder(ETY, ETZ, EUA).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(EUN))) -> gleam@erlang@process:selector(message(EUN)).
...
-spec receive_message(self(any(), EUJ)) -> message(EUJ).
...
-spec initialise_actor(
    builder(any(), any(), EVG),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EVG} | {error, binary()})
...
-spec start(builder(any(), any(), EVT)) -> {ok, started(EVT)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EWA), EWA) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EWC),
    integer(),
    fun((gleam@erlang@process:subject(EWE)) -> EWC)
) -> EWE.
