...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(ALPW))
) -> glint:command(ALPW).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(ALQN)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, ALQN} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, ALQN} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(ALQW))
) -> glint:command(ALQW).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(ALPQ))
) -> glint:command(ALPQ).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(ALQD))
) -> glint:command(ALQD).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(ALQJ))
) -> glint:command(ALQJ).
