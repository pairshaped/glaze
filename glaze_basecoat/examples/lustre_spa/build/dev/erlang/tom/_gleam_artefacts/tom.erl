...
-spec push_key({ok, AHHQ} | {error, get_error()}, binary()) -> {ok, AHHQ} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {AHJV, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AHJV, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {AHKG, list(binary())}} | {error, parse_error()},
    fun((AHKG, list(binary())) -> {ok, AHKJ} | {error, parse_error()})
) -> {ok, AHKJ} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {AHKO, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AHKO, list(binary())}} | {error, parse_error()}.
