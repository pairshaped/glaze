...
pub fn main() {
  let assert Ok(watcher) =
    ...
    |> polly.add_callback(fn(event) {
      case event {
        polly.Changed(path) -> io.println("CHANGED " <> path)
        polly.Created(path) -> io.println("CREATED " <> path)
        polly.Deleted(path) -> io.println("DELETED " <> path)
        polly.Error(_, _) -> Nil
      }
    })
    |> polly.watch()

  // Watcher runs in the background, stop her when you're done
  // polly.stop(watcher)
}
```
