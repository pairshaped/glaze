...
-opaque app(XHK, XHL, XHM) :: {app,
        fun((XHK) -> {XHL, lustre@effect:effect(XHM)}),
        fun((XHL, XHM) -> {XHL, lustre@effect:effect(XHM)}),
        fun((XHL) -> lustre@vdom@vnode:element(XHM))}.

-opaque simulation(XHN, XHO) :: {simulation,
        fun((XHN, XHO) -> {XHN, lustre@effect:effect(XHO)}),
        fun((XHN) -> lustre@vdom@vnode:element(XHO)),
        list(event(XHO)),
        XHN,
        lustre@vdom@vnode:element(XHO)}.

-type event(XHP) :: {dispatch, XHP} |
...
-spec simple(
    fun((XHQ) -> XHR),
    fun((XHR, XHS) -> XHR),
    fun((XHR) -> lustre@vdom@vnode:element(XHS))
) -> app(XHQ, XHR, XHS).
...
-spec application(
    fun((XHX) -> {XHY, lustre@effect:effect(XHZ)}),
    fun((XHY, XHZ) -> {XHY, lustre@effect:effect(XHZ)}),
    fun((XHY) -> lustre@vdom@vnode:element(XHZ))
) -> app(XHX, XHY, XHZ).
...
-spec start(app(XIG, XIH, XII), XIG) -> simulation(XIH, XII).
start(App, Args) ->
...
-spec message(simulation(XIO, XIP), XIP) -> simulation(XIO, XIP).
...
-spec problem(simulation(XJU, XJV), binary(), binary()) -> simulation(XJU, XJV).
...
-spec model(simulation(XKA, any())) -> XKA.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), XKF)) -> lustre@vdom@vnode:element(XKF).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), XKK)) -> list(event(XKK)).
history(Simulation) ->
...
-spec click(simulation(XJB, XJC), lustre@dev@query:'query'()) -> simulation(XJB, XJC).
...
-spec input(simulation(XJH, XJI), lustre@dev@query:'query'(), binary()) -> simulation(XJH, XJI).
