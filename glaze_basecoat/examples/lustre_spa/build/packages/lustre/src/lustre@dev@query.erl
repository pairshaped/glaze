...
-file("src/lustre/dev/query.gleam", 58).
?DOC(
...
-file("src/lustre/dev/query.gleam", 96).
?DOC(
...
-file("src/lustre/dev/query.gleam", 108).
?DOC(
...
-file("src/lustre/dev/query.gleam", 126).
?DOC(
...
-file("src/lustre/dev/query.gleam", 155).
?DOC(
...
-file("src/lustre/dev/query.gleam", 170).
?DOC(
...
-file("src/lustre/dev/query.gleam", 178).
?DOC(
...
-file("src/lustre/dev/query.gleam", 185).
?DOC(
...
-file("src/lustre/dev/query.gleam", 200).
?DOC(
...
-file("src/lustre/dev/query.gleam", 208).
?DOC(
...
-file("src/lustre/dev/query.gleam", 223).
?DOC(
...
-file("src/lustre/dev/query.gleam", 255).
?DOC(
...
-file("src/lustre/dev/query.gleam", 627).
...
text_content(Element, Inline, Content) ->
    case Element of
        ...
        {map, _, _, _, Child@1} ->
            text_content(Child@1, Inline, Content);

        {memo, _, _, _, View} ->
            text_content(View(), Inline, Content);

        {element, _, _, _, _, _, _, _, _, _} when not Inline orelse (erlang:element(
            4,
            Element
        ) =/= <<""/utf8>>) ->
            gleam@list:fold(
                ...
                Content,
                fun(Content@2, Child@2) ->
                    text_content(Child@2, true, Content@2)
                end
            );

        {element, _, _, _, <<"a"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
                    text_content(Child@3, true, Content@3)
                end
            );

        {element, _, _, _, <<"abbr"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"acronym"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"b"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"bdo"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"big"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"br"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"button"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"cite"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"code"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"dfn"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"em"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"i"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"img"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"input"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"kbd"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"label"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"map"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"object"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"output"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"q"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"samp"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"script"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"select"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"small"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"span"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"strong"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"sub"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"sup"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"textarea"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"time"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"tt"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
        ...
        {element, _, _, _, <<"var"/utf8>>, _, _, _, _, _} ->
            gleam@list:fold(
                erlang:element(7, Element),
                Content,
                fun(Content@3, Child@3) ->
            ...
            case Is_inline of
                true ->
                    gleam@list:fold(
                        erlang:element(7, Element),
                        Content,
                        fun(Content@4, Child@4) ->
                            text_content(Child@4, true, Content@4)
                        end
                    );
        ...
        {text, _, _, _} ->
            <<Content/binary, (erlang:element(4, Element))/binary>>;

        {unsafe_inner_html, _, _, _, _, _, _} ->
            Content
    end.

-file("src/lustre/dev/query.gleam", 553).
...
matches(Element, Selector) ->
    case {Element, Selector} of
        ...
        {{element, _, _, Namespace, Tag, _, _, _, _, _}, {type, _, _}} ->
            (Namespace =:= erlang:element(2, Selector)) andalso (Tag =:= erlang:element(
                3,
                Selector
            ));

        {{unsafe_inner_html, _, _, Namespace, Tag, _, _}, {type, _, _}} ->
            (Namespace =:= erlang:element(2, Selector)) andalso (Tag =:= erlang:element(
                3,
                Selector
            ));

        {{element, _, _, _, _, Attributes, _, _, _, _},
        ...
        {{unsafe_inner_html, _, _, _, _, Attributes, _},
            {has_attribute, Name, <<""/utf8>>}} ->
            gleam@list:any(Attributes, fun(Attribute) -> case Attribute of
                        {attribute, _, _, _} ->
                            erlang:element(3, Attribute) =:= Name;
        ...
        {{element, _, _, _, _, Attributes@1, _, _, _, _},
        ...
        {{unsafe_inner_html, _, _, _, _, Attributes@1, _},
            {has_attribute, Name@1, Value}} ->
            gleam@list:contains(
                Attributes@1,
                lustre@attribute:attribute(Name@1, Value)
        ...
        {{element, _, _, _, _, Attributes@2, _, _, _, _}, {has_class, Name@2}} ->
        ...
        {{unsafe_inner_html, _, _, _, _, Attributes@2, _}, {has_class, Name@2}} ->
            gleam@list:fold_until(
                gleam@string:split(Name@2, <<" "/utf8>>),
                true,
                fun(_, Class) ->
        ...
        {{element, _, _, _, _, Attributes@3, _, _, _, _},
        ...
        {{unsafe_inner_html, _, _, _, _, Attributes@3, _},
            {has_style, Name@4, Value@2}} ->
            Rule = <<<<<<Name@4/binary, ":"/utf8>>/binary, Value@2/binary>>/binary,
                ";"/utf8>>,
            gleam@list:any(Attributes@3, fun(Attribute@2) -> case Attribute@2 of
        ...
        {{element, _, _, _, _, _, _, _, _, _}, {contains, Content}} ->
            _pipe = Element,
...
-file("src/lustre/dev/query.gleam", 354).
-spec find_matching_in_list(
    list(lustre@vdom@vnode:element(WTW)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(WTW), lustre@vdom@path:path()}} |
    {error, nil}.
find_matching_in_list(Elements, Selector, Path, Index) ->
    case Elements of
        ...
        [{fragment, _, _, _, _} = First | Rest] ->
            find_matching_in_list(
                lists:append(erlang:element(4, First), Rest),
                Selector,
...
-file("src/lustre/dev/query.gleam", 338).
-spec find_direct_child(
    lustre@vdom@vnode:element(WTR),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTR), lustre@vdom@path:path()}} |
    {error, nil}.
find_direct_child(Parent, Selector, Path) ->
    case Parent of
        {element, _, _, _, _, _, Children, _, _, _} ->
            find_matching_in_list(Children, Selector, Path, 0);

        {fragment, _, _, Children, _} ->
            find_matching_in_list(Children, Selector, Path, 0);

        {map, _, _, _, Child} ->
            find_direct_child(Child, Selector, Path);

        {memo, _, _, _, View} ->
            find_direct_child(View(), Selector, Path);
...
-file("src/lustre/dev/query.gleam", 494).
-spec find_all_matching_in_list(
    list(lustre@vdom@vnode:element(WVE)),
    selector()
) -> list(lustre@vdom@vnode:element(WVE)).
...
-spec find_all_direct_children(lustre@vdom@vnode:element(WVA), selector()) -> list(lustre@vdom@vnode:element(WVA)).
find_all_direct_children(Parent, Selector) ->
    case Parent of
        ...
        {map, _, _, _, Child} ->
            find_all_direct_children(Child, Selector);

        {memo, _, _, _, View} ->
            find_all_direct_children(View(), Selector);
...
-file("src/lustre/dev/query.gleam", 765).
...
-file("src/lustre/dev/query.gleam", 724).
?DOC(
...
-file("src/lustre/dev/query.gleam", 464).
-spec find_all_in_list(list(lustre@vdom@vnode:element(WUV)), 'query'()) -> list(lustre@vdom@vnode:element(WUV)).
...
-file("src/lustre/dev/query.gleam", 426).
?DOC(
    " Like [`find`](#find) but returns every element in the view that matches the\n"
    " given query.\n"
).
-spec find_all(lustre@vdom@vnode:element(WUN), 'query'()) -> list(lustre@vdom@vnode:element(WUN)).
find_all(Root, Query) ->
    case Query of
        {find_element, Selector} ->
...
-file("src/lustre/dev/query.gleam", 449).
-spec find_all_in_children(lustre@vdom@vnode:element(WUR), 'query'()) -> list(lustre@vdom@vnode:element(WUR)).
find_all_in_children(Element, Query) ->
    case Element of
        ...
        {map, _, _, _, Child} ->
            find_all_in_children(Child, Query);

        {memo, _, _, _, View} ->
            find_all_in_children(View(), Query);
...
-file("src/lustre/dev/query.gleam", 526).
-spec find_all_descendants_in_list(
    list(lustre@vdom@vnode:element(WVN)),
    selector()
) -> list(lustre@vdom@vnode:element(WVN)).
...
-file("src/lustre/dev/query.gleam", 508).
-spec find_all_descendants(lustre@vdom@vnode:element(WVJ), selector()) -> list(lustre@vdom@vnode:element(WVJ)).
find_all_descendants(Parent, Selector) ->
    ...
    Descendant_matches = case Parent of
        ...
        {map, _, _, _, Child} ->
            find_all_descendants(Child, Selector);

        {memo, _, _, _, View} ->
            find_all_descendants(View(), Selector);
...
-file("src/lustre/dev/query.gleam", 320).
-spec find_in_list(
    list(lustre@vdom@vnode:element(WTL)),
    'query'(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(WTL), lustre@vdom@path:path()}} |
    {error, nil}.
...
-file("src/lustre/dev/query.gleam", 278).
?DOC(false).
-spec find_path(
    lustre@vdom@vnode:element(WTB),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTB), lustre@vdom@path:path()}} |
    {error, nil}.
...
-file("src/lustre/dev/query.gleam", 305).
-spec find_in_children(
    lustre@vdom@vnode:element(WTG),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTG), lustre@vdom@path:path()}} |
    {error, nil}.
find_in_children(Element, Query, Index, Path) ->
    case Element of
        ...
        {map, _, _, _, Child} ->
            find_in_children(Child, Query, Index, Path);

        {memo, _, _, _, View} ->
            find_in_children(View(), Query, Index, Path);
...
-file("src/lustre/dev/query.gleam", 265).
?DOC(
...
-spec find(lustre@vdom@vnode:element(WSW), 'query'()) -> {ok,
        lustre@vdom@vnode:element(WSW)} |
    {error, nil}.
find(Root, Query) ->
...
-file("src/lustre/dev/query.gleam", 399).
-spec find_descendant_in_list(
    list(lustre@vdom@vnode:element(WUH)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(WUH), lustre@vdom@path:path()}} |
    {error, nil}.
...
-file("src/lustre/dev/query.gleam", 379).
-spec find_descendant(
    lustre@vdom@vnode:element(WUC),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WUC), lustre@vdom@path:path()}} |
    {error, nil}.
    ...
    case find_direct_child(Parent, Selector, Path) of
        ...
        {error, _} ->
            case Parent of
                ...
                {map, _, _, _, Child} ->
                    find_descendant(Child, Selector, Path);

                {memo, _, _, _, View} ->
                    find_descendant(View(), Selector, Path);
